...
VoidResult ScreenRecorderWindows::create_device() {
  ...
  HRESULT hres = D3D11CreateDevice(nullptr, // Use default adapter
                                   D3D_DRIVER_TYPE_HARDWARE, // Hardware device
                                   nullptr, // No software rasterizer
                                   0,       // No flags
                                   nullptr,
                                   0, // No feature level array – pick default
                                   D3D11_SDK_VERSION, &d3d_device_,
                                   &feature_lvl, &d3d_context_);

  if (FAILED(hres)) {
    return std::unexpected(
        make_error().with_context("Failed to initialize D3D11 device"));
  }
...
VoidResult ScreenRecorderWindows::create_desk_duplication() {
  ...
  if (FAILED(hres)) {
    return std::unexpected(
        make_error().with_context("Failed to get DXGI device"));
  }
  ...
  if (FAILED(hres)) {
    return std::unexpected(
        make_error().with_context("Failed to get DXGI adapter"));
  }
  ...
  if (FAILED(hres)) {
    return std::unexpected(make_error().with_context(
        std::format("Failed to enumerate output #{}", config_.monitor_index_)));
  }
  ...
  if (FAILED(hres)) {
    return std::unexpected(
        make_error().with_context(std::format("Failed to get IDXGIOutput1")));
  }
  ...
  if (FAILED(hres)) {
    return std::unexpected(
        make_error().with_context(std::format("Failed duplicating output")));
  }
  ...
  if (FAILED(hres)) {
    return std::unexpected(make_error().with_context(
        std::format("Failed to acquire the first frame")));
  }
  ...
  if (FAILED(hres)) {
    ...
    return std::unexpected(make_error().with_context(
        std::format("Failed to convert first frame to a texture")));
  }
  ...
  if (frame_width != static_cast<UINT>(width_) ||
      frame_height != static_cast<UINT>(height_)) {
    Log::video_capture()->debug("Resolution mismatch detected at startup: "
                                "expected {}x{} – got {}x{}",
                                width_, height_, frame_width, frame_height);
...
VoidResult ScreenRecorderWindows::create_staging_texture() {
  ...
  HRESULT hres =
      d3d_device_->CreateTexture2D(&staging_desc, nullptr, &staging_texture_);
  if (FAILED(hres)) {
    return std::unexpected(make_error().with_context(
        std::format("Failed to create staging texture")));
  }
...
VoidResult ScreenRecorderWindows::init_encoder() {
  ...
  if (auto res = encoder_queue_.initialize(encoder_cfg); !res) {
    return std::unexpected(make_error().with_context(std::format(
        "Failed to initialise encoder at startup: {}", res.error().what())));
  }
...
void ScreenRecorderWindows::capture_frame() {
  ...
  if (monitor_->need_video_reinit_.exchange(false,
                                            std::memory_order::acq_rel)) {
    if (auto res = reinit_duplication(); !res) {
      Log::video_capture()->warn("Failed reiniting duplication: {}",
                                 res.error().what());
      monitor_->need_video_reinit_.store(true, std::memory_order::release);
    } else {
      Log::video_capture()->debug(
          "Reinitialized duplication context after lock");
    }
  ...
  if ((desk_dupl_ == nullptr) || (staging_texture_ == nullptr) ||
  ...
  HRESULT hres =
      desk_dupl_->AcquireNextFrame(0, &frame_info, &desktop_resource);
  ...
  if (hres == DXGI_ERROR_WAIT_TIMEOUT) {
    ...
    if (last_frame_ != nullptr) {
      if (auto res = encoder_queue_.push_frame(last_frame_, final_stride_, pts);
          !res) {
  ...
  if (FAILED(hres)) {
    Log::video_capture()->warn(
        "Failed converting generic DXGI resource into a D3D11 texture");
  ...
  hres = d3d_context_->Map(staging_texture_.Get(), 0, D3D11_MAP_READ, 0,
                           &mapped_resource);
  ...
  int64_t pts = get_next_frame_pts();
  last_frame_ =
      prepare_frame_data(static_cast<uint8_t *>(mapped_resource.pData),
                         mapped_resource.RowPitch, final_stride_);
...
Core::RotationType ScreenRecorderWindows::get_rotation_type(int rotation) {
  ...
  case 1:
    return Core::RotationType::kRotationIdentity;
  case 2:
    return Core::RotationType::kRotationRotate90;
  case 3:
    return Core::RotationType::kRotationRotate180;
  case 4:
    return Core::RotationType::kRotationRotate270;
  }
...
void ScreenRecorderWindows::push_black_frame(int64_t pts) {
  ...
  if (auto res = encoder_queue_.push_frame(last_frame_, final_stride_, pts);
      !res) {
    Log::video_capture()->trace("Failed pushing BLACK frame: {}",
                                res.error().what());
  } else {
