...
VoidResult ScreenRecorderLinux::init_platform() {
  if (XInitThreads() == 0) {
    Log::video_capture()->warn(
        "XInitThreads() failed; Xlib calls may not be thread-safe");
  }
  ...
  if (display_ == nullptr) {
    return std::unexpected(
        make_error().with_context("Failed to open X display"));
  }
  ...
  if (rotation_ != Core::RotationType::kRotationIdentity &&
      rotation_ != Core::RotationType::kUnspecified) {
    size_t buffer_size = width_ * height_ * 4; // BGRA format
    rotation_buffer_.resize(buffer_size);
    Log::video_capture()->info("Allocated rotation buffer: {} bytes",
                               buffer_size);
  }
  ...
  if (XShmQueryVersion(display_, &major, &minor, &pixmaps) != 0) {
    ...
    ximage_ = XShmCreateImage(display_, DefaultVisual(display_, screen_num_),
                              DefaultDepth(display_, screen_num_), ZPixmap,
                              nullptr, &shminfo_, width_, height_);

    if (ximage_ == nullptr) {
      // If creating a SHM-backend image failed, fallback to standard path.
      Log::video_capture()->warn(
          "Failed to create SHM image, falling back to standard XGetImage");
      use_shm_ = false;
    } else {
      ...
      if (shminfo_.shmid < 0) {
        ...
        shminfo_.readOnly =
            False; // We will write to it (X server writes, we read)
    ...
    Log::video_capture()->info(
        "MIT-SHM extension not available, using standard XGetImage");
    use_shm_ = false;
  }
...
Result<Core::RotationType> ScreenRecorderLinux::detect_screen_rotation() {
  ...
  if (XRRQueryExtension(display_, &event_base, &error_base) == 0) {
    return std::unexpected(
        make_error().with_context("XRandR extension not available"));
  }
  ...
  if (XRRQueryVersion(display_, &major_version, &minor_version) == 0) {
    return std::unexpected(
        make_error().with_context("Failed to query XRandR version"));
  }

  Log::video_capture()->debug("XRandR version: {}.{}", major_version,
                              minor_version);

  // Get screen resources
  XRRScreenResources *screen_resources =
      XRRGetScreenResourcesCurrent(display_, root_window_);
  if (screen_resources == nullptr) {
    return std::unexpected(
        make_error().with_context("Failed to get XRandR screen resources"));
  }
  ...
  XRROutputInfo *output_info =
      XRRGetOutputInfo(display_, screen_resources, primary);
  if (output_info == nullptr) {
    XRRFreeScreenResources(screen_resources);
    return std::unexpected(
        make_error().with_context("Failed to get output info"));
  }
  ...
  if (output_info->connection != RR_Connected || output_info->crtc == None) {
    ...
    XRRFreeScreenResources(screen_resources);
    return std::unexpected(
        make_error().with_context("Output is not connected or has no CRTC"));
  }
  ...
  if (crtc_info == nullptr) {
    ...
    XRRFreeScreenResources(screen_resources);
    return std::unexpected(
        make_error().with_context("Failed to get CRTC info"));
  }
  ...
  // XRandR uses bit flags: RR_Rotate_0, RR_Rotate_90, RR_Rotate_180,
  // RR_Rotate_270
...
void ScreenRecorderLinux::capture_frame() {
  ...
  if (use_shm_) {
    ...
    if (XShmGetImage(display_, root_window_, ximage_, 0, 0, AllPlanes) == 0) {
      ...
      if (last_frame_ != nullptr) {
        Log::video_capture()->trace(
            "Reusing last frame due to capture failure");
    ...
    image = XGetImage(display_, root_window_, 0, 0, width_, height_, AllPlanes,
                      ZPixmap);
    if (image == nullptr) {
  ...
  if (image->bits_per_pixel != 32) {
    Log::video_capture()->error("Expected 32bpp, got {}",
  ...
  last_frame_ = prepare_frame_data(reinterpret_cast<uint8_t *>(image->data),
                                   image->bytes_per_line, final_stride_);
