...
WinSessionPowerMonitor::~WinSessionPowerMonitor() {
    stop();
}
...
VoidResult WinSessionPowerMonitor::start() {
    ...
    th_ = std::jthread([this](std::stop_token stoken) {
        ...
        // PeekMessageW ensures the queue exists so PostThreadMessageW will work reliably
        MSG msg{};
...
LRESULT CALLBACK WinSessionPowerMonitor::WndProc(HWND hwnd, UINT msg, WPARAM wpar, LPARAM lpar) {
    WinSessionPowerMonitor* self = nullptr;

    if (msg == WM_NCCREATE) {
        // WM_NCCREATE is the first message during window creation.
        // lpar contains the CREATESTRUCTW*, where lpCreateParams is what we passed to CreateWindowExW
    ...
    self = reinterpret_cast<WinSessionPowerMonitor*>(GetWindowLongPtrW(hwnd, GWLP_USERDATA));
    if (self == nullptr) {
        return DefWindowProcW(hwnd, msg, wpar, lpar);
    }
    ...
    case WM_POWERBROADCAST: {
        ...
        if (wpar == PBT_APMRESUMESUSPEND || wpar == PBT_APMRESUMEAUTOMATIC) {
            // System resumed hence desktop duplication will require an re-initialization
            self->need_video_reinit_.store(true, std::memory_order::release);
            self->force_black_flag_.store(false, std::memory_order::release);
        }
    ...
    default: return DefWindowProcW(hwnd, msg, wpar, lpar);
    }
}
    ...
    if (RegisterClassExW(&wc) == 0U) {
        ...
        if (err != ERROR_CLASS_ALREADY_EXISTS) {
            return std::unexpected(
                make_error().with_context(std::format("RegisterClassExW failed: {}", (unsigned)err)));
        }
    }

    // Creates a window. We use HWND_MEESSAGE to create a
    // "message-only" window which is not visible not on taskbar and exists purely to reveive messages
    hwnd_ = CreateWindowExW(0, kClassName, L"", 0, 0, 0, 0, 0, HWND_MESSAGE, nullptr, wc.hInstance, this);
    if (hwnd_ == nullptr) {
        const DWORD err = GetLastError();
        return std::unexpected(make_error().with_context(std::format("CreateWindowExW failed: {}", (unsigned)err)));
    }
