...
ApiServer::ApiServer(asio::io_context& ioc, const std::string& address, unsigned short port, AppContext app_context)
    : acceptor_(ioc)
    ...
    if (err) {
        Log::api()->critical("Bind error error: {} (binding {}:{})", err.message(), address, port);
        std::quick_exit(EXIT_FAILURE);
        return;
    }
...
void ApiServer::do_accept() {
    acceptor_.async_accept([this](beast::error_code errc, tcp::socket socket) {
        if (!errc) {
            std::make_shared<HttpSession>(std::move(socket), app_context_)->run();
        }
        do_accept();
    });
}