...
private:
    void do_read() {
        asio::async_read_until(
            ...
            asio::bind_executor(
                (strand_),
                [self = shared_from_this()](boost::system::error_code errc, std::size_t bytes) {
                    if (errc) {
                        Log::sync()->warn("Sync worker session read error: {}", errc.message());
                        self->close();
                        return;
                    }
    ...
    void handle_line(const std::string& line) {
        ...
        if (type_str == "ping") {
            const int64_t t_1 = (obj.if_contains("t1") != nullptr) ? obj["t1"].as_int64() : 0;
            const int seq = (obj.if_contains("seq") != nullptr) ? static_cast<int>(obj["seq"].as_int64()) : 0;
    ...
    void do_write() {
        ...
        asio::async_write(
            ...
            asio::bind_executor(
                strand_,
                [self = shared_from_this(), keep](boost::system::error_code errc, std::size_t /* bytes */) {
                    if (errc) {
                        self->cleanup();
                        return;
                    }
...
SyncMasterServer::SyncMasterServer(asio::io_context& io_ctx, std::string bind_ip, unsigned short port)
    : io_ctx_(io_ctx)
    , acceptor_(io_ctx)
...
void SyncMasterServer::do_accept() {
    acceptor_.async_accept([this](boost::system::error_code errc, tcp::socket sock) {
        if (!errc) {
            boost::system::error_code ep_ec;
            const auto remote = sock.remote_endpoint(ep_ec);

            if (!ep_ec) {
                Log::sync()->info("Sync worker connected from: {}:{}", remote.address().to_string(), remote.port());
            }
            else {
...
void SyncMasterServer::send_export_at(int64_t t_master_ns, const std::string& output_path) {
    auto worker = worker_.lock();
    if (!worker) {
    ...
    timer_.async_wait([this, output_path](const boost::system::error_code& /* errc */) {
        if (!media_recorder_) {
            return;
        }
        ...
        if (auto res = media_recorder_->save_and_upload_async(); !res) {
            Log::sync()->error("Failed saving and uploading in audio master recorder: {}", res.error().what());
            return;
        }
...
void SyncMasterServer::on_worker_connected() {
    ...
    timer_.async_wait([this](const boost::system::error_code& /* errc */) { on_warmup_timer(); });
}
...
void SyncMasterServer::on_warmup_timer() {
    ...
    Log::sync()->info("Scheduling synchrnoized start at master unix ns={}", scheduled_t0_master_ns);
    ...
    timer_.async_wait([this](const boost::system::error_code& /* errc */) { on_start_timer(); });
}
void SyncMasterServer::on_start_timer() {
    ...
    if (auto res = media_recorder_->start(); !res) {
        Log::sync()->error("Failed starting audio master recorder: {}", res.error().what());
        return;
    }

    Log::sync()->info("Audio master recorder started");
}
void SyncMasterServer::enable_auto_start(std::shared_ptr<Core::MediaRecorder> media_recorder, double seg_seconds) {
    media_recorder_ = std::move(media_recorder);
    seg_seconds_ = seg_seconds;
}
