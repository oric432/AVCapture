...
Error::VoidResult
RollingSegment::initialize(const Config &cfg,
...
Error::VoidResult RollingSegment::create_segment_dir() {
  ...
  if (errc) {
    return std::unexpected(Error::make_error().with_context(
        std::format("create directories failed with: {}", errc.message())));
  }
  return {};
}
...
void RollingSegment::remove_segment_dir() noexcept {
  ...
  if (errc) {
    Log::media_recorder()->warn("Failed to remove temp folder '{}': {}",
                                config_.dir_.string(), errc.message());
  }
}
  ...
  th_ = std::jthread([this](std::stop_token st) {
    while (!st.stop_requested()) {
      ...
      if (auto res = tick_save_one_segment(); !res) {
        Log::media_recorder()->error("Segment tick failed: {}",
                                     res.error().what());
      }
    }
  });
...
RollingSegment::export_last_segments(size_t segments,
  ...
  if (errc) {
    return std::unexpected(
        Error::make_error().with_context("Failed removing temp list.txt"));
  }
...
Error::VoidResult RollingSegment::tick_save_one_segment() {
  const auto idx = static_cast<size_t>(seq_.load(std::memory_order_relaxed) %
                                       config_.ring_size_);
  ...
  Log::media_recorder()->info("Saved segment {} : {}", idx,
                              path.string().c_str());
...
Error::VoidResult
RollingSegment::save_av_to_ts(const std::filesystem::path &ts_path) {
  ...
  if (video_frames.empty()) {
    return std::unexpected(
        Error::make_error().with_context("No video frames to write"));
  }

  AVFormatContext *fmt_ctx = nullptr;
  if (avformat_alloc_output_context2(&fmt_ctx, nullptr, "mpegts",
                                     ts_path.string().c_str()) < 0 ||
      (fmt_ctx == nullptr)) {
    return std::unexpected(
        Error::make_error().with_context("Failed allocating format context"));
  }
  ...
  if (audio_frames.empty()) {
    return std::unexpected(
        Error::make_error().with_context("No audio frames to write"));
  }
  ...
  if (audio_stream == nullptr) {
    return std::unexpected(
        Error::make_error().with_context("Failed to create audio stream"));
  }
  ...
  if (avcodec_parameters_from_context(audio_stream->codecpar,
    ...
    return std::unexpected(Error::make_error().with_context(
        "Failed to copy audio copy parameters"));
  }
  ...
  if ((fmt_ctx->oformat->flags & AVFMT_NOFILE) == 0 &&
      (fmt_ctx->pb == nullptr)) {
    auto res = avio_open2(&fmt_ctx->pb, ts_path.string().c_str(),
                          AVIO_FLAG_WRITE, nullptr, nullptr);
    char errbuf[128];
    av_strerror(res, errbuf, sizeof(errbuf));
    if (res < 0) {
      return std::unexpected(Error::make_error().with_context(std::format(
          "Failed opening file handle for {}: {}", ts_path.string(), errbuf)));
    }
  }
  ...
  if (avformat_write_header(fmt_ctx, nullptr) < 0) {
    return std::unexpected(
        Error::make_error().with_context("Failed writing file header"));
  }
  ...
  int64_t video_frames_diff =
      video_frames.back().pts_ - static_cast<int64_t>(video_frames.size() - 1);
  int64_t audio_frames_diff = audio_frames.back().pts_ -
                              (static_cast<int64_t>(audio_frames.size() - 1) *
  ...
  Log::media_recorder()->debug("Muxing {} video frames and {} audio frames",
  ...
  while (video_idx < video_frames.size() && audio_idx < audio_frames.size()) {
    ...
    if (av_compare_ts(video_pts, video_codec_context->time_base, audio_pts,
                      audio_codec_context->time_base) <= 0) {
      ...
      if (auto res = write_packet_to_stream(
              fmt_ctx, video_stream, video_codec_context->time_base,
              frame.data_.data(), frame.data_.size(), video_pts, video_dts,
              is_key_frame);
          !res) {
        Log::media_recorder()->error(
            "Failed to write video packet at index {} - {}", video_idx,
            res.error().what());
        break;
      }
      ...
      if (auto res = write_packet_to_stream(
              fmt_ctx, audio_stream, audio_codec_context->time_base,
              frame.data_.data(), frame.data_.size(), audio_pts, audio_dts,
              frame.is_key_frame_);
          !res) {
        Log::media_recorder()->error(
            "Failed to write audio packet at index {} - {}", video_idx,
            res.error().what());
        break;
      }
...
Error::VoidResult RollingSegment::write_packet_to_stream(
    AVFormatContext *fmt_ctx, AVStream *stream, AVRational src_time_base,
    const uint8_t *data, size_t size, int64_t pts, int64_t dts,
    bool is_key_frame) {
