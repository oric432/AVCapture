...
Result<std::string> ArtifactExporter::save_and_upload(
    RollingSegment& segmenter,
    Nfs::IFileBackend& nfs) {
    auto bundle = prepare();
    if (!bundle) {
        return std::unexpected(bundle.error().with_context("Failed creating bundle paths"));
    }
...
VoidResult ArtifactExporter::export_mp4(RollingSegment& segmenter, const BundlePaths& bundle_paths) const {
    if (auto res = segmenter.export_last_segments(bundle_paths.segments_to_export_, bundle_paths.output_mp4_); !res) {
        return std::unexpected(res.error().with_context("Failed exporting segments"));
    }

    return {};
}

static VoidResult upload_zip_streaming(
    Nfs::IFileBackend& nfs,
    ...
    if (!input.is_open()) {
        return std::unexpected(
            make_error().with_context(std::format("Failed opening local zip on: '{}'", local_zip.string())));
    }

    auto file = nfs.open_write_trunc(remote_tmp);
    if (!file) {
        return std::unexpected(file.error().with_context("Failed opening file on nfs"));
    }
    ...
    while (input) {
        ...
        if (!write) {
            if (auto res = nfs.close(*file); !res) {
                return std::unexpected(write.error().with_context(
                    std::format("Write failed and failed closing file : {}", res.error().what())));
            }

            return std::unexpected(write.error().with_context("Failed writing to nfs"));
        }
    }
    ...
    if (auto res = nfs.rename(remote_tmp, remote_final); !res) {
        return std::unexpected(res.error().with_context("Failed renaming temp file"));
    }
...
VoidResult ArtifactExporter::zip_bundle(const BundlePaths& bundle_paths) const {
    ...
    if (errc || size < 1024) {
        return std::unexpected(make_error().with_context(std::format("Zip file empty: {}", errc.message())));
    }
...
VoidResult ArtifactExporter::upload(Nfs::IFileBackend& nfs, const BundlePaths& bundle_paths) const {
    ...
    if (auto upload =
            upload_zip_streaming(nfs, bundle_paths.zip_path_, bundle_paths.remote_tmp_, bundle_paths.remote_final_);
        !upload) {
        return std::unexpected(upload.error().with_context("Failed uploading zip to nfs"));
    }
    return {};
}
