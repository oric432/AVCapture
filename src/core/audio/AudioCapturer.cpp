...
VoidResult AudioCapturer::initialize(const Core::AudioConfig &config) {
  if (is_initialized_) {
    return std::unexpected(make_error().with_context(
        "Failed to initialize since object is already initialized"));
  }
  ...
  if (auto res = audio_mixer_.initialize(config_); !res) {
    return std::unexpected(
        res.error().with_context("Failed to initialize audio mixer"));
  }
  ...
  if (system_device_id_ < 0) {
    ...
    system_audio_.setErrorCallback(
        [](RtAudioErrorType /**/, const std::string & /**/) { return; });

    if (auto res = audio_mixer_.add_stream(system_device_id_); !res) {
      Log::audio_capture()->warn("Failed adding stream to mixer: {}",
                                 res.error().what());
    }
  }
  ...
  if (system_device_id_ < 0 && mic_device_id_ < 0) {
    return std::unexpected(make_error().with_context(
        "Neither input and output devices were found"));
  }
...
VoidResult AudioCapturer::start() {
  if (!is_initialized_) {
    return std::unexpected(make_error().with_context(
        "Failed to start since object is not initialized"));
  }

  if (is_capturing_) {
    return std::unexpected(make_error().with_context(
        "Failed to start since capture threads are already running"));
  }
  ...
  if (system_device_id_ >= 0) {
    ...
    if (system_audio_.openStream(
            nullptr, &params, RTAUDIO_SINT16, config_.sample_rate_,
            &config_.buffer_frame_size_, &AudioCapturer::system_audio_callback,
            this) != 0) {
      return std::unexpected(
          make_error().with_context("Failed to open system audio"));
    }

    if (system_audio_.startStream() != 0) {
      return std::unexpected(
          make_error().with_context("Failed starting system capture"));
    }
  }
  ...
  if (mic_device_id_ >= 0 && system_device_id_ != mic_device_id_) {
    ...
    if (mic_audio_.openStream(nullptr, &params, RTAUDIO_SINT16,
                              config_.sample_rate_, &config_.buffer_frame_size_,
                              &AudioCapturer::mic_audio_callback, this) != 0) {
      return std::unexpected(
          make_error().with_context("Failed to open mic audio"));
    }

    if (mic_audio_.startStream() != 0) {
      return std::unexpected(
          make_error().with_context("Failed starting mic capture"));
    }
  }
...
void AudioCapturer::stop() {
  ...
  is_capturing_ = false;
  Log::audio_capture()->info(
      "Audio capture stopped. Total sampled captured: {}", total_samples_);
}
bool AudioCapturer::is_capturing() { return is_capturing_; }

int AudioCapturer::system_audio_callback(
    void * /*output_buffer*/, void *input_buffer, unsigned int n_frames,
  ...
  if (input_buffer != nullptr) {
    ...
    auto raw_audio_frame =
        std::make_unique<RawAudioFrame>(data, samples_count, 0);

    if (auto res = capturer->audio_mixer_.push_audio(
            capturer->system_device_id_, std::move(raw_audio_frame));
        res) {
      Log::audio_capture()->trace("Pushed {} system frames successfully",
                                  n_frames);
    } else {
      Log::audio_capture()->warn("Could not push audio to queue: {}",
                                 res.error().what());
    }
  }
...
int AudioCapturer::mic_audio_callback(void * /*output_buffer*/,
                                      void *input_buffer, unsigned int n_frames,
                                      double /*stream_time*/,
  ...
  if (input_buffer != nullptr) {
    ...
    if (auto res = capturer->audio_mixer_.push_audio(
            capturer->mic_device_id_, std::move(raw_audio_frame));
        res) {
      Log::audio_capture()->trace("Pushed {} mic frames successfully",
                                  n_frames);
    } else {
      Log::audio_capture()->warn("Could not push audio to queue: {}",
                                 res.error().what());
...
int AudioCapturer::find_loopback_device() {
  ...
  if (default_output >= 0) {
    ...
    Log::audio_capture()->warn(
        "No specific loopback device found, using default input device: {}",
        info.name);
