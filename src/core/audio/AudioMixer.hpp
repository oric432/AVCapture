...
public:
  ...
  Error::VoidResult push_audio(const unsigned int device_id,
                               std::unique_ptr<RawAudioFrame> frame);

  std::vector<EncodedAudioFrame> get_audio_buffer() const {
    return audio_buffer_.snapshot();
  }
  void clear_frames() { audio_buffer_.clear(); }
  AVCodecContext *get_codec_context() const {
    return audio_encoder_.has_value() ? audio_encoder_->get_codec_context()
                                      : nullptr;
  }

private:
  using AudioStream =
      moodycamel::ReaderWriterQueue<std::unique_ptr<RawAudioFrame>>;

  void wait_for_all_stream(std::stop_token stop_token);
  void mixer_loop(std::stop_token stop_token);

  static void mix_samples(std::vector<int16_t> &output,
