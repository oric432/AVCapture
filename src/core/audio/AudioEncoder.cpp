...
Result<AudioEncoder> AudioEncoder::create(const Core::AudioConfig& config) {
    ...
    if (auto res = audio_encoder.initialize(config); !res) {
        return std::unexpected(res.error().with_context("Failed initializing audio encoder"));
    }
...
VoidResult AudioEncoder::initialize(const Core::AudioConfig& config) {
    ...
    if (codec_ctx_ == nullptr) {
        return std::unexpected(make_error().with_context("Could not allocate codec context"));
    }
    ...
    if (avcodec_open2(codec_ctx_, codec_, nullptr) < 0) {
        cleanup();
        return std::unexpected(make_error().with_context("Failed to open audio codec"));
    }
    ...
    if (ret < 0 || swr_ctx_ == nullptr) {
        cleanup();
        return std::unexpected(make_error().with_context("Failed to allocated audio resampler"));
    }

    if (swr_init(swr_ctx_) < 0) {
        cleanup();
        return std::unexpected(make_error().with_context("Failed to initialize audio resampler"));
    }
...
Result<bool>
AudioEncoder::encode_frame(const int16_t* pcm_data, int32_t sample_count, int64_t pts, EncodedAudioFrame& out_frame) {
    if (codec_ctx_ == nullptr) {
        return std::unexpected(make_error().with_context("codec was not initialized"));
    }

    if (swr_ctx_ == nullptr) {
        return std::unexpected(make_error().with_context("swr was not initialized"));
    }

    auto* frame = av_frame_alloc();
    if (frame == nullptr) {
        return std::unexpected(make_error().with_context("Failed allocating frame"));
    }
    ...
    if (av_frame_get_buffer(frame, 0) < 0) {
        cleanup();
        return std::unexpected(make_error().with_context("Failed to allocated audio frame buffer"));
    }

    const uint8_t* input[1] = {reinterpret_cast<const uint8_t*>(pcm_data)};

    int converted = swr_convert(swr_ctx_, frame->data, frame->nb_samples, input, sample_count);

    if (converted < 0) {
        return std::unexpected(make_error().with_context("Audio resampling failed"));
    }
    else if (converted == 0) {
        return std::unexpected(make_error().with_context("Could not convert samples"));
    }
    ...
    if (pkt == nullptr) {
        return std::unexpected(make_error().with_context("ffmpeg packet was not initialized"));
    }
    ...
    if (ret < 0) {
        av_packet_free(&pkt);
        av_frame_free(&frame);
        return std::unexpected(make_error().with_context("Failed to send audio frame to encoder"));
    }
    ...
    if (ret == AVERROR_EOF) {
        ...
        av_packet_free(&pkt);
        av_frame_free(&frame);
        return std::unexpected(make_error().with_context("Failed to recieve audio packet"));
    }
...
// Drain any remaining packets from the encoder to flush buffered data before freeing the context
VoidResult AudioEncoder::drain() {
    int ret = avcodec_send_frame(codec_ctx_, nullptr);

    if (ret < 0) {
        return std::unexpected(make_error().with_context("Failed sending nullptr frame to encoder"));
    }

    AVPacket* pkt = av_packet_alloc();
    if (pkt == nullptr) {
        return std::unexpected(make_error().with_context("Failed allocating new packet"));
    }
    ...
    for (;;) {
        ...
        av_packet_free(&pkt);
        return std::unexpected(make_error().with_context("Failed receiving packet from encoder"));
    }
...
void AudioEncoder::cleanup() {
    ...
    if (codec_ctx_ != nullptr) {
        if (auto res = drain(); !res) {
            Log::audio_encode()->warn("Failed flushing encoder: {}", res.error().what());
        }
