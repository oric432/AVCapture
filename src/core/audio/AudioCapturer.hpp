...
public:
  ...
  std::vector<EncodedAudioFrame> get_frames() const {
    return audio_mixer_.get_audio_buffer();
  }
  void clear_frames() { audio_mixer_.clear_frames(); }
  AVCodecContext *get_encoder_context() const {
    return audio_mixer_.get_codec_context();
  }

private:
  static void rt_audio_callback(RtAudioErrorType /**/,
                                const std::string & /**/) {};

  static int system_audio_callback(void *output_buffer, void *input_buffer,
                                   unsigned int n_frames, double stream_time,
                                   RtAudioStreamStatus status, void *user_data);

  static int mic_audio_callback(void *output_buffer, void *input_buffer,
                                unsigned int n_frames, double stream_time,
                                RtAudioStreamStatus status, void *user_data);
  ...
  std::atomic<bool> is_capturing_{
      false}; // For system audio (loopback/WASAPI/monitor)
