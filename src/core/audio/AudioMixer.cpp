...
VoidResult AudioMixer::initialize(const Core::AudioConfig &config) {
  ...
  if (!res) {
    return std::unexpected(
        res.error().with_context("Failed to initialize audio encoder"));
  }

  audio_encoder_.emplace(std::move(res.value()));

  const auto total_sample_target = static_cast<int64_t>(
      std::llround(config_.recording_length_seconds_ * config_.sample_rate_));

  const auto max_frames = static_cast<size_t>(
      (total_sample_target + config_.buffer_frame_size_ - 1) /
      config_.buffer_frame_size_);

  audio_buffer_.set_capacity(max_frames);

  const double actual_duration =
      static_cast<double>(max_frames * config_.buffer_frame_size_) /
      static_cast<double>(config_.sample_rate_);

  Log::audio_capture()->debug(
      "Audio buffer configured: {:.1f}s duration, {} frames ({}HHz, "
      "{} frame size)",
      actual_duration, max_frames, config_.sample_rate_,
      config_.buffer_frame_size_);
...
void AudioMixer::start() {
  ...
  mixer_thread_ = std::jthread(
      [this](std::stop_token stop_token) { mixer_loop(stop_token); });
}
...
VoidResult AudioMixer::add_stream(const unsigned int device_id) {
  if (audio_streams_.find(device_id) != audio_streams_.end()) {
    return std::unexpected(
        make_error().with_context("Audio stream already exists"));
  }
...
VoidResult AudioMixer::push_audio(const unsigned int device_id,
                                  std::unique_ptr<RawAudioFrame> frame) {
  auto stream = audio_streams_.find(device_id);
  if (stream == audio_streams_.end()) {
    return std::unexpected(
        make_error().with_context("Audio stream already exists"));
...
void AudioMixer::mixer_loop(std::stop_token stop_token) {
  std::vector<int16_t> mixed_buffer(config_.buffer_frame_size_ *
                                    config_.channels_);

  // TODO Can i use a vector
  std::unordered_map<unsigned int, std::unique_ptr<RawAudioFrame>>
      current_frames;
  ...
  while (!stop_token.stop_requested()) {
    ...
    auto res = audio_encoder_->encode_frame(
        mixed_buffer.data(), static_cast<int32_t>(mixed_buffer.size()), pts_,
        encoded_frame);
    if (!res) {
      Log::audio_encode()->warn("Failed encoding audio frame: {}",
                                res.error().what());
    }
    // Check if value true or false, if false = AVERROR / AVEAGAIN, no need to
    // push new packet
    else if (res.value()) {
...
void AudioMixer::mix_samples(std::vector<int16_t> &output,
                             const boost::span<const int16_t> &input) {
  ...
  for (size_t i = 0; i < samples_to_mix; ++i) {
    ...
    static constexpr LargerSampleType kMIN_SAMPLE =
        std::numeric_limits<int16_t>::min();
    static constexpr LargerSampleType kMAX_SAMPLE =
        std::numeric_limits<int16_t>::max();
    const auto clipped_sample = std::max<int32_t>(
        kMIN_SAMPLE, std::min<int32_t>(mixed_sample, kMAX_SAMPLE));
