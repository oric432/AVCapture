...
VideoEncoderQueue::VideoEncoderQueue()
    : frame_queue_(
          std::make_unique<
              moodycamel::ReaderWriterQueue<std::unique_ptr<RawVideoFrame>>>(
              1024)) // Pre-allocate space for 1024 frames
{}

VideoEncoderQueue::~VideoEncoderQueue() { stop(); }
...
VoidResult VideoEncoderQueue::initialize(const Core::VideoConfig &config) {
  ...
  const auto max_frames =
      static_cast<size_t>(config_.recording_length_seconds_ * config_.fps_);
  ...
  Log::video_encode()->debug(
      ...
      "({}fps, {}x{})",
      config_.recording_length_seconds_, max_frames, config_.fps_,
...
void VideoEncoderQueue::start() {
  ...
  encoding_thread_ = std::jthread(
      [this](std::stop_token stop_token) { encoding_loop(stop_token); });
...
VoidResult VideoEncoderQueue::push_frame(const uint8_t *data, int stride,
                                         int64_t pts) {
  ...
  return std::unexpected(make_error().with_context(
      std::format("Frame queue full, dropping frame with PTS {}", pts)));
}
...
void VideoEncoderQueue::encoding_loop(std::stop_token stop_token) {
  ...
  while (!stop_token.stop_requested()) {
    ...
    if (frame_queue_->try_dequeue(frame)) {
      ...
      if (auto res = video_encoder_->encode_frame(
              frame->data_.data(), frame->stride_, frame->pts_, encoded_frame);
          !res) {
        Log::video_encode()->error("Failed to encode frame with PTS {} - {}",
                                   frame->pts_, res.error().what());
      } else {
