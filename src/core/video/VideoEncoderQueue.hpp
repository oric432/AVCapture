...
public:
   ...
   */
  std::vector<EncodedVideoFrame> get_encoded_frames() const {
    return encoded_buffer_.snapshot();
  }
  ...
  AVCodecContext *get_encoder_context() const {
    return video_encoder_.has_value() ? video_encoder_->get_codec_context()
                                      : nullptr;
  }
...
private:
  ...
  std::unique_ptr<moodycamel::ReaderWriterQueue<std::unique_ptr<RawVideoFrame>>>
      frame_queue_;
