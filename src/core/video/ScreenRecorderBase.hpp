...
public:
  ...
  std::vector<EncodedVideoFrame> get_frames() const override {
    return encoder_queue_.get_encoded_frames();
  }
  void clear_frames() override { encoder_queue_.clear_frames(); }
  AVCodecContext *get_encoder_context() const override {
    return encoder_queue_.get_encoder_context();
  }

  uint8_t *prepare_frame_data(uint8_t *src_data, int src_stride,
                              int &out_stride) override;
