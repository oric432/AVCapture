...
VoidResult VideoEncoder::initialize(const Core::VideoConfig& config) {
    ...
    const std::array<const char*, 3> encoders = {
    ...
    for (const char* encoder : encoders) {
        if (auto res = try_open_encoder(encoder, config); res) {
            ...
            if (frame_ == nullptr) {
                return std::unexpected(make_error().with_context("Failed to allocate frame for video encoder"));
            }
            ...
            if (av_frame_get_buffer(frame_, 32) < 0) {
                return std::unexpected(make_error().with_context("Failed to allocate frame buffer for video encoder"));
            }

            // Allocate a reusable packet to receive encoded date from the encoder
            packet_ = av_packet_alloc();
            if (packet_ == nullptr) {
                return std::unexpected(make_error().with_context("Failed to allocate packet for video encoder"));
            }
            ...
            if (sws_ctx_ == nullptr) {
                return std::unexpected(make_error().with_context("Failed to initialize scaler for video encoder"));
            }
    ...
    return std::unexpected(make_error().with_context("No H.264 encoder could be opened"));
}

VoidResult VideoEncoder::try_open_encoder(const char* encoder_name, const Core::VideoConfig& config) {
    ...
    // Makes SPS/PPS appear in-band (ofter before keyframes) which helps with segments that starts with P-frames
    // So we avoid ffmpeg errors upon saving a segment
    ...
    if (err < 0) {
        ...
        return std::unexpected(
            make_error().with_context(std::format("Failed to open encoder {} : {}", encoder_name, err_buf.data())));
    }
...
VoidResult
VideoEncoder::encode_frame(uint8_t* src_data, int src_line_size, int64_t pts, EncodedVideoFrame& encoded_frame) {
    ...
    sws_scale(sws_ctx_, src.data(), src_stride.data(), 0, height_, frame_->data, frame_->linesize);
    ...
    if (ret < 0) {
        return std::unexpected(make_error().with_context("Failed sending frame to encoder context"));
    }

    // Try to receive a packet. not all calls produce one (pipelined encoders)
    ret = avcodec_receive_packet(codec_ctx_, packet_);
    if (ret == AVERROR(EAGAIN) || ret == AVERROR_EOF) {
        return std::unexpected(make_error().with_context("Failed receiving packet from encoder context"));
    }

    if (ret < 0) {
        return std::unexpected(make_error().with_context(std::format("Error receiving encoded packet: {}", ret)));
    }
