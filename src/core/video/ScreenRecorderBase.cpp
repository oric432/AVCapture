...
VoidResult ScreenRecorderBase::initialize(const Core::VideoConfig &config) {
  ...
  if (auto res = init_platform(); !res) {
    return std::unexpected(res.error().with_context(
        "Failed initializing platform specific screen recorder"));
  }
  ...
  if (rotation_ == Core::RotationType::kRotationRotate90 ||
      rotation_ == Core::RotationType::kRotationRotate270) {
  ...
  if (auto res = encoder_queue_.initialize(encoder_config); !res) {
    ...
    return std::unexpected(
        res.error().with_context("Failed initializing video encoder thread"));
  }
...
VoidResult ScreenRecorderBase::start() {
  ...
  if (is_recording_.load(std::memory_order::relaxed)) {
    return std::unexpected(make_error().with_context(
        "Failed to start since recording thread is already running"));
  }
  ...
  record_thread_ =
      std::jthread([this](std::stop_token st) { recording_loop(st); });
...
void ScreenRecorderBase::recording_loop(std::stop_token st) {
  ...
  const auto frame_duration =
      std::chrono::microseconds(1'000'000 / config_.fps_);
...
uint8_t *ScreenRecorderBase::prepare_frame_data(uint8_t *src_data,
                                                int src_stride,
                                                int &out_stride) {
  // No rotation needed
  if (rotation_ == Core::RotationType::kUnspecified ||
  ...
  if (rotation_ ==
      Core::RotationType::kRotationRotate90) { // 90 degrees clockwise
    ...
    for (int y = 0; y < height_; ++y) {
      ...
      for (int x = 0; x < width_; ++x) {
        uint32_t *dst_pixel =
            (uint32_t *)(dst + x * height_ * 4 + (height_ - 1 - y) * 4);
        uint32_t *src_pixel = (uint32_t *)(src_row + x * 4);
        *dst_pixel = *src_pixel;
      }
    }
  } else if (rotation_ ==
             Core::RotationType::kRotationRotate180) { // 180 degrees clockwise
    out_stride = width_ * 4;
    ...
    for (int y = 0; y < height_; ++y) {
      ...
      for (int x = 0; x < width_; ++x) {
        ...
        *dst_pixel = *src_pixel;
             ...
             Core::RotationType::kRotationRotate270) { // 270 degrees clockwise
    ...
    for (int y = 0; y < height_; ++y) {
      ...
      for (int x = 0; x < width_; ++x) {
        ...
        uint32_t *dst_pixel =
            (uint32_t *)(dst + (width_ - 1 - x) * height_ * 4 + y * 4);
        uint32_t *src_pixel = (uint32_t *)(src_row + x * 4);
        *dst_pixel = *src_pixel;
      }
    }
